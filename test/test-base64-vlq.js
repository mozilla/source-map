...
function decodeMapping(aStr) {
  ...
  for (let idx = 0; idx <= len; ++idx) {
    ...
    if (c === 44 || c === 59) {
      ...
      console.log({generatedLine, sourceIndex, sourceLine, sourceCol, nameIndex});
      mappings.push([generatedLine, sourceIndex, sourceLine, sourceCol, nameIndex]);
  ...
  // First: Sort by generated line.
  mappings.sort((a, b) => b[0] - a[0]);
  console.log(mappings);
  console.log(copyToInt32(mappings));
  // Second: Sort by source line.
  mappings.sort((a, b) => b[2] - a[2]);
  console.log(mappings);
  console.log(copyToInt32(mappings));
...
function copyToInt32(arr) {
  const result = new Int32Array(arr.length * 5);
  let offset = 0;
  for (const mapping of arr) {
    result.set(mapping, offset, 5);
    offset += 5;
  }
  return result;
}
...
exports["test normal encoding and decoding"] = function(assert) {
  for (let i = 0; i < vlqs.length; i++) {
    ...
    const n = decodeMapping(vlqs[i].encoded);
    assert.equal(
      vlqs[i].number,
      n,
      `number ${vlqs[i].number} should decode correctly: ${vlqs[i].number} == ${n}`
    );
  }

  console.log(decodeMapping(`A,AKAK;A,ACAK`));
};
