...
const wasm = require("./wasm-sync");

const INTERNAL = Symbol("smcInternal");

class SourceMapConsumerSync {
  ...
  static async with(rawSourceMap, sourceMapUrl, f) {
    const consumer = new SourceMapConsumerSync(rawSourceMap, sourceMapUrl);
    try {
 ...
 */
SourceMapConsumerSync.prototype._version = 3;
SourceMapConsumerSync.GENERATED_ORDER = 1;
SourceMapConsumerSync.ORIGINAL_ORDER = 2;

SourceMapConsumerSync.GREATEST_LOWER_BOUND = 1;
SourceMapConsumerSync.LEAST_UPPER_BOUND = 2;

exports.SourceMapConsumerSync = SourceMapConsumerSync;
 ...
 */
class BasicSourceMapConsumerSync extends SourceMapConsumerSync {
  constructor(aSourceMap, aSourceMapURL) {
    super(INTERNAL);
    const that = this;
   ...
   */
  static fromSourceMap(aSourceMap, aSourceMapURL) {
    return new BasicSourceMapConsumerSync(aSourceMap.toString());
  }
  ...
  eachMapping(aCallback, aContext, aOrder) {
    ...
    const order = aOrder || SourceMapConsumerSync.GENERATED_ORDER;
    ...
    this._wasm.withMappingCallback(
      ...
      () => {
        switch (order) {
          case SourceMapConsumerSync.GENERATED_ORDER:
            this._wasm.exports.by_generated_location(this._getMappingsPtr());
            break;
          case SourceMapConsumerSync.ORIGINAL_ORDER:
  ...
  originalPositionFor(aArgs) {
    ...
    let bias = util.getArg(aArgs, "bias", SourceMapConsumerSync.GREATEST_LOWER_BOUND);
    if (bias == null) {
      bias = SourceMapConsumerSync.GREATEST_LOWER_BOUND;
    }
...
BasicSourceMapConsumerSync.prototype.consumer = SourceMapConsumerSync;
exports.BasicSourceMapConsumerSync = BasicSourceMapConsumerSync;
 ...
 */
class IndexedSourceMapConsumerSync extends SourceMapConsumerSync {
  constructor(aSourceMap, aSourceMapURL) {
    super(INTERNAL);
    const that = this;
    ...
    that._sections = sections.map(s => {
      ...
      const cons = new SourceMapConsumerSync(util.getArg(s, "map"), aSourceMapURL);
      return {
        generatedOffset: {
...
exports.IndexedSourceMapConsumerSync = IndexedSourceMapConsumerSync;
...
function _factory(aSourceMap, aSourceMapURL) {
  ...
  const consumer = sourceMap.sections != null
    ? new IndexedSourceMapConsumerSync(sourceMap, aSourceMapURL)
    : new BasicSourceMapConsumerSync(sourceMap, aSourceMapURL);
  return consumer;
}

function _factoryBSM(aSourceMap, aSourceMapURL) {
  return BasicSourceMapConsumerSync.fromSourceMap(aSourceMap, aSourceMapURL);
}
